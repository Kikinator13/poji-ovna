<?php
    class personManager{
         ...
         */
        public function addPerson($person,  $userId, $addressId) : int
        {
            $person["user"]=$userId;
         ...
         */
        public function updatePerson(array $person) : bool
        {
            $id = array_pop($person);
            
            try{ 
                $countRows = Mysql::update('persons', $person, "persons_id = ?", array($id));
        ...
        public function updateAddress(array $address, $personId) : bool
        {
            ...
            try{
                ...
                throw new UserException("Chyba databáze při úpravě adresy!".$error->getMessage(), 40401, $error);
            }
            if ($countRows == 1){
                return true;
            }else{
        ...
        public function getPerson(int $id) : bool|array
        {
            try{
            $users = Mysql::oneRow(
                'SELECT persons_id, first_name, last_name, users.user, `admin`, street, house_number, building_identification_number, city, ZIP, national_id_number, identity_card_number
